...
export default function About() {
  ...
  { 
    name: "Tunca Tosun", // Updated Name
    role: "Founder", 
    sub: "Lead Engineer", 
    image: "/founder3.jpeg", 
    linkedin: "http://linkedin.com/in/tunca-tosun-9098bb192" // Removed space
  },
];
          ...
          <div className="flex flex-col sm:flex-row flex-wrap justify-center items-center gap-12">
            {team.map((member, index) => (
              <div key={index} className="w-full max-w-[320px]">
                ...
                <div className="aspect-[4/5] bg-[#12181f] border border-gray-800 rounded-2xl mb-6 flex items-center justify-center overflow-hidden relative shadow-2xl transition-all duration-500 hover:border-[#ccff00]/50">
                  {member.image ? (
                    <img 
                      src={member.image} 
                      alt={member.name} 
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    {/* CHANGED THIS FROM DIV TO <a> FOR CLICKABLE LINK */}
                    <a 
                      href={member.linkedin}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-5 h-5 bg-gray-800 rounded flex items-center justify-center text-[#ccff00] text-[10px] font-bold cursor-pointer hover:bg-[#007bff] hover:text-white transition-colors"
                    >
                       in
                    </a>
                  </div>
